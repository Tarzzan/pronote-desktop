  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) {
          setEntries(archive);
          setOpened(archive[0]?.period.id ?? null);
        }
      } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {entries.map((entry) => {
            const isOpen = opened === entry.period.id;
            return (
              <section key={entry.period.id} className="bg-white border border-gray-200 rounded-xl shadow-sm">
                <button
                  onClick={() => setOpened((prev) => (prev === entry.period.id ? null : entry.period.id))}
